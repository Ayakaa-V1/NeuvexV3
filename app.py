...
PERUBAHAN v2.1:
    - Binance API DIHAPUS (diblokir di Indonesia → Error 451)
    - Crypto sekarang pakai CoinGecko Public API (gratis, no key, no block)
    - CoinGecko rate limit ~30 req/menit → dihandle cache 120 detik
"""
...
<style>
  ...
  .level-label { font-size: 0.65rem; color: #6e7681; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px; }
...
# ─────────────────────────────────────────────────────────────────────
CRYPTO_ASSETS = {
    "BTC (Bitcoin)":  {"cg": "bitcoin",  "label": "BTC/USDT"},
    "ETH (Ethereum)": {"cg": "ethereum", "label": "ETH/USDT"},
    "SOL (Solana)":   {"cg": "solana",   "label": "SOL/USDT"},
}
FOREX_ASSETS = {
...
# ─────────────────────────────────────────────────────────────────────
# DATA FETCHING — CRYPTO via CoinGecko (GRATIS, NO KEY, NO GEO-BLOCK)
# ─────────────────────────────────────────────────────────────────────
CG_HEADERS = {
    "User-Agent": "Mozilla/5.0 NeuroBro/2.1",
    "Accept": "application/json",
}

@st.cache_data(ttl=120)
def fetch_crypto_coingecko(cg_id: str) -> dict:
    """
    Ambil harga + 24h stats dari CoinGecko Public API.
    Tidak butuh API key, tidak diblokir di Indonesia.
    Rate limit: ~30 req/menit → aman dengan cache 120 detik.
    """
    try:
        url = (
        ...
        r = requests.get(url, headers=CG_HEADERS, timeout=10)
        r.raise_for_status()
        data = r.json()
        if not data:
            return {"ok": False, "error": "CoinGecko: data kosong"}
        d = data[0]
        price  = float(d["current_price"])
        high   = float(d["high_24h"])
        low    = float(d["low_24h"])
        open_  = float(d.get("price_change_24h", 0)) + price  # approx open
        chg    = float(d["price_change_percentage_24h"] or 0)
...
def fetch_crypto_history_cg(cg_id: str, days: int = 30) -> pd.DataFrame:
    """
    Ambil OHLC historis dari CoinGecko (gratis, tanpa key).
    CoinGecko OHLC endpoint: interval otomatis berdasarkan 'days'.
    """
    try:
        url = (
        ...
        r = requests.get(url, headers=CG_HEADERS, timeout=12)
        r.raise_for_status()
        raw = r.json()
        if not raw:
            return pd.DataFrame()
...
def fetch_yf_quote(ticker: str) -> dict:
    """Fetch latest quote dari Yahoo Finance."""
    try:
        tk   = yf.Ticker(ticker)
...
@st.cache_data(ttl=300)
def fetch_yf_history(ticker: str, period: str = "60d", interval: str = "1d") -> pd.DataFrame:
    try:
        tk = yf.Ticker(ticker)
...
# ─────────────────────────────────────────────────────────────────────
# QUANT ENGINE — INDIKATOR TEKNIKAL
# ─────────────────────────────────────────────────────────────────────
...
def compute_macd(series: pd.Series):
    ...
    hist   = macd_l - sig
    return float(macd_l.iloc[-1]), float(sig.iloc[-1]), float(hist.iloc[-1])
...
def run_neurobro_analysis(quote: dict, df: pd.DataFrame) -> dict:
    ...
    if df.empty or len(df) < 26:
        ...
        if sma20 > sma50:
            signals["SMA Cross"] = (f"SMA20 > SMA50", "bull"); score += 1
        else:
            signals["SMA Cross"] = (f"SMA20 < SMA50", "bear"); score -= 1
    ...
    return {
        "decision": decision, "d_class": d_class, "score": score,
        "signals": signals, "entry": entry, "tp": tp, "sl": sl,
        "rr": round(rr, 2), "vol_pct": vol, "narrative": narrative,
    }
...
# NEWS ENGINE
# ─────────────────────────────────────────────────────────────────────
BULLISH_KW = ["surge","rally","gain","rise","bull","buy","growth","record","boost","breakout","recovery","strong","upside","profit","soar"]
BEARISH_KW = ["fall","drop","crash","sell","bear","decline","recession","fear","loss","plunge","weak","risk","concern","warning","inflation","hawkish"]
...
def fetch_rss_news() -> list:
    headers  = {"User-Agent": "Mozilla/5.0 NeuroBro/2.1"}
    articles = []
    for feed_url in RSS_FEEDS:
        try:
            ...
            continue
    return articles[:10] if articles else _simulated_news()

def _simulated_news() -> list:
    templates = [
        ...
        ("Ethereum ecosystem sees record daily active addresses", "BULLISH"),
    ...
    return [{"title": t, "sentiment": s, "pub": now} for t, s in templates[:8]]
...
<div class="nb-header">
  ...
  <div style="margin-left:auto;display:flex;gap:8px;flex-wrap:wrap;">
    <span class="nb-badge">v2.1 Indonesia Fix</span>
    <span class="nb-badge">CoinGecko</span>
...
with col_left:
    ...
    cat = st.selectbox("Select Market", ["CRYPTO", "FOREX & GOLD", "STOCKS"], label_visibility="collapsed")
    ...
    if cat == "CRYPTO":
        for name, cfg in CRYPTO_ASSETS.items():
            q = fetch_crypto_coingecko(cfg["cg"])
            if q["ok"]:
                st.metric(label=cfg["label"], value=f"${q['price']:,.2f}", delta=f"{q['change_pct']:+.2f}%")
            else:
                st.metric(label=name, value="—", delta=f"⚠ {q.get('error','Error')[:30]}")
        ...
        for name, cfg in FOREX_ASSETS.items():
            ...
            if q["ok"]:
                p = q["price"]
                if "Gold" in name:
                    st.metric(label=cfg["label"], value=f"${p:,.2f}", delta=f"{q['change_pct']:+.3f}%")
                elif "JPY" in name:
                    st.metric(label=cfg["label"], value=f"{p:.3f}", delta=f"{q['change_pct']:+.3f}%")
                else:
                    st.metric(label=cfg["label"], value=f"{p:.5f}", delta=f"{q['change_pct']:+.3f}%")
            else:
    ...
    <div style="margin-top:14px;padding:10px 14px;background:#0d1117;
      ...
      <div style="font-size:0.65rem;color:#6e7681;font-family:'JetBrains Mono',monospace;line-height:1.8;">
        🟢 Crypto: CoinGecko API (cache 120s)<br>
        🔄 Forex/Stock: Yahoo Finance (cache 120s)<br>
        🔄 History: cache 300s<br>
        ⚡ Anti-429 · Anti-451 AKTIF
      </div>
    </div>
    """, unsafe_allow_html=True)
...
with col_mid:
    ...
    if cat == "CRYPTO":
        ...
        quote = fetch_crypto_coingecko(cfg["cg"])
        df    = fetch_crypto_history_cg(cfg["cg"], days=30) if quote.get("ok") else pd.DataFrame()
        price_label = cfg["label"]
    elif cat == "FOREX & GOLD":
        cfg   = FOREX_ASSETS.get(selected, list(FOREX_ASSETS.values())[0])
        quote = fetch_yf_quote(cfg["yf"])
        df    = fetch_yf_history(cfg["yf"]) if quote.get("ok") else pd.DataFrame()
        price_label = cfg["label"]
    else:
    ...
    if not quote.get("ok"):
        st.error(f"⚠️ Data unavailable: {quote.get('error','Unknown')}. Tunggu ~30 detik lalu refresh.")
        st.info("💡 Tip: Jika error terus, klik 'Force Refresh All Data' di kolom kanan.")
        st.stop()
    ...
    <div class="{css_class}">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
        ...
        <div>
          ...
          <div class="decision-label {label_cls}">{ana['decision']}</div>
        </div>
      ...
      <div class="level-grid">
        <div class="level-box">
          ...
          <div class="level-entry">{ana['entry']:,.{dp}f}</div>
        </div>
        <div class="level-box">
          <div class="level-label">🎯 TAKE PROFIT</div>
          <div class="level-tp">{ana['tp']:,.{dp}f}</div>
        </div>
        <div class="level-box">
          <div class="level-label">🛑 STOP LOSS</div>
          <div class="level-sl">{ana['sl']:,.{dp}f}</div>
        </div>
      </div>

      <div style="display:flex;gap:12px;margin-top:12px;flex-wrap:wrap;">
        <div style="background:#0d1117;border:1px solid #21262d;border-radius:6px;padding:6px 12px;
        ...
        <div style="background:#0d1117;border:1px solid #21262d;border-radius:6px;padding:6px 12px;
             font-size:0.72rem;font-family:'JetBrains Mono',monospace;">
          Src &nbsp;<span style="color:#6e7681;">{quote['source']}</span>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for name, (val, sig) in ana["signals"].items():
        ico = "▲" if sig == "bull" else ("▼" if sig == "bear" else "◆")
        ind_html += f'<div class="ind-row"><span class="ind-name">{name}</span><span class="ind-val {sig}">{ico} {val}</span></div>'
    ...
    if not df.empty and len(df) >= 5:
        ...
        if not chart_df.empty:
            ...
            st.line_chart(chart_df.tail(60), height=200, use_container_width=True)
...
with col_right:
    ...
    pct        = int(bull_count / max(len(news), 1) * 100)
    mc         = "#3fb950" if overall == "BULLISH" else "#f85149"

    st.markdown(f"""
    <div style="background:#161b22;border:1px solid #21262d;border-radius:10px;
         padding:14px 18px;margin-bottom:14px;">
      <div style="font-size:0.65rem;color:#6e7681;font-family:'JetBrains Mono',monospace;
           text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px;">Overall Sentiment</div>
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="font-size:1.3rem;font-weight:700;font-family:'JetBrains Mono',monospace;color:{mc};">{overall}</div>
        <div style="flex:1;">
        ...
        <div style="font-size:0.8rem;font-family:'JetBrains Mono',monospace;color:{mc};">{pct}%</div>
      </div>
      <div style="display:flex;gap:14px;margin-top:8px;">
        <div style="font-size:0.7rem;font-family:'JetBrains Mono',monospace;color:#3fb950;">▲ BULL {bull_count}</div>
        <div style="font-size:0.7rem;font-family:'JetBrains Mono',monospace;color:#f85149;">▼ BEAR {bear_count}</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for item in news:
        ...
        <div class="news-item">
          ...
          <div class="news-meta">{badge} &nbsp; {item['pub']}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style="margin-top:16px;padding:12px 16px;background:#0d1117;
         border:1px solid #21262d;border-radius:8px;">
      <div style="font-size:0.65rem;color:#6e7681;line-height:1.7;font-family:'JetBrains Mono',monospace;">
        ⚠️ <strong style="color:#d29922;">DISCLAIMER</strong><br>
        Output NeuroBro hanya untuk tujuan edukasi. Bukan saran investasi.
        Selalu lakukan riset mandiri (DYOR) dan kelola risiko Anda.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="margin-top:10px;font-size:0.66rem;color:#6e7681;
      ...
      Last render: {datetime.utcnow().strftime('%H:%M:%S')} UTC
    </div>
    """, unsafe_allow_html=True)

    auto = st.checkbox("⚡ Auto-refresh (120s)", value=False)
    if auto:
        st.markdown('<div style="font-size:0.68rem;color:#3fb950;font-family:monospace;margin-top:6px;">● Active</div>', unsafe_allow_html=True)
        time.sleep(120)
        st.cache_data.clear()
        st.rerun()
...
st.markdown("---")
st.markdown("""
<div style="text-align:center;padding:12px;font-size:0.68rem;color:#6e7681;font-family:'JetBrains Mono',monospace;">
  🧠 NeuroBro v2.1 · CoinGecko + Yahoo Finance + Python Quant Engine · Streamlit ·
  <span style="color:#3fb950;">100% FREE · NO API KEY · WORKS IN INDONESIA</span>
</div>
""", unsafe_allow_html=True)
